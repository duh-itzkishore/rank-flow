...
import { Calendar, Download, Loader2, TrendingUp, TrendingDown, BarChart3, LayoutGrid, BarChart2 } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { ContentArchitecture } from "@/modules/Planning & Creation/ContentArchitecture";
import { ChartGenerator } from "@/modules/Planning & Creation/ChartGenerator";

export const Route = createFileRoute("/app/$projectId/analytics")({
  component: Analytics,
});
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("prompt_runs")
        .select("id, model, is_mentioned, rank, created_at, prompts!inner(project_id)")
        .eq("prompts.project_id", selectedProjectId)
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-white tracking-tight print:text-black">Analytics & Content Planning</h1>
          <p className="mt-1 text-sm text-white/40 print:text-black/60">AI visibility trends, share of voice & content mapping</p>
        </div>
      ...
      {/* Tab Switcher */}
      <div className="flex items-center gap-1 rounded-xl bg-white/[0.03] ring-1 ring-white/[0.06] p-1 w-fit print:hidden">
        {[
          { key: "analytics", label: "Performance Analytics", icon: BarChart3 },
          { key: "planner", label: "Content Planner", icon: LayoutGrid },
          { key: "chart", label: "AI Chart Generator", icon: BarChart2 },
